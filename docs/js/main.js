(() => {
  ...
  if (slider) {
    const slides = Array.from(slider.querySelectorAll(".city-slide"));
    const dotsWrap = document.getElementById("citySliderDots");
    const labels = Array.from(document.querySelectorAll("#citySliderLabels span"));
    const progress = document.getElementById("citySliderProgress");
    const progressBar = progress && progress.parentElement;
    const prevBtn = document.getElementById("citySliderPrev");
    const nextBtn = document.getElementById("citySliderNext");
    const reduceMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
    let index = 0;
    let timer = null;
    const INTERVAL = 5500;
    ...
    slides.forEach((_, i) => {
      const dot = document.createElement("button");
      dot.type = "button";
      dot.className = "city-slider__dot" + (i === 0 ? " is-active" : "");
      dot.setAttribute("aria-label", `Слайд ${i + 1}`);
      dot.addEventListener("click", () => goTo(i));
      dotsWrap.appendChild(dot);
    });
    const dots = Array.from(dotsWrap.querySelectorAll(".city-slider__dot"));
    ...
    labels[0] && labels[0].classList.add("is-active");
    restartProgress();
    resetTimer();
  }
